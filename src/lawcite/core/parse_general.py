...
def parse_general_paragraphs(pdf: PdfReader) -> Dict[str, str]:
    ...
    current_content = []
    in_body = False
    document_title = pdf.metadata.get("/Title", "") if pdf.metadata and "/Title" in pdf.metadata else ""
    ...
    for page_num, page in enumerate(pdf.pages):
        ...
        for i, line in enumerate(lines):
            ...
            if page_num == 0 and not in_body:
                if re.match(r"^\d+\.\s|^[A-Z][a-z]+\s*[-–]\s*[A-Z][a-z]+|^Udskriftsdato:|^VEJ nr|^Ministerium:", line) or line == document_title:
                    continue
                in_body = True
